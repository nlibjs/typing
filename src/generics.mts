...
export type UndefinedAsOptional<T extends object> =
	OptionalKeys<T> extends never
		...
		: RequiredKeys<T> extends never
			? { [K in OptionalKeys<T>]?: ProcessUndefined<T[K]> }
			: {
					[K in RequiredKeys<T>]: ProcessUndefined<T[K]>;
				} & { [K in OptionalKeys<T>]?: ProcessUndefined<T[K]> };
export type TypeGuard<T> = (input: unknown) => input is T;
type ProcessUndefined<T> = T extends
	| bigint
	| number
	| string
	| symbol
	...
	| undefined
	? T
	: T extends DefinitionEnum<infer S>
		? ProcessUndefined<S>
		: T extends DefinitionCandidates<infer S>
			? ProcessUndefined<S>
			: T extends DefinitionConditions<infer S>
				? ProcessUndefined<S>
				: T extends Array<infer S>
					? Array<ProcessUndefined<S>>
					: T extends Set<infer S>
						? Set<ProcessUndefined<S>>
						: T extends Map<infer S, infer V>
							? Map<S, ProcessUndefined<V>>
							: T extends object
								? UndefinedAsOptional<T>
								: T;
export type DefinedType<T> = T extends Definition<infer S>
	? ProcessUndefined<S>
	: never;
export type GuardedType<T> = T extends TypeGuard<infer S>
	? ProcessUndefined<S>
	: never;
