...
import {listIpv4Octets} from './parseIpv4Address';
...
export type Ipv6AddressGroups = [number, number, number, number, number, number, number, number];
...
export const listIpv6Groups = function* (input: string, fromIndex = 0): Generator<Ipv6AddressGroup> {
    let start = fromIndex;
    let length = 0;
    let value = 0;
...
export const parseIpv6Address = (input: string, start = 0): Ipv6AddressParseResult => {
    ...
    for (const group of listIpv6Groups(input, start)) {
        ...
        if (value === null) {
            ...
            if (0 <= compressorIndex) {
                throw new Error(`DuplicatedCompressor: ${input.slice(start, group.end)}`);
            } else {
